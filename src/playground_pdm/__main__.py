...
def log():
    def log_wrapper(func):
        ...
        def wrap(*args, **kwargs):
            logger.remove()
            log_format: str = '<level>{level}</level> | <cyan>{extra[func]}</cyan>:<cyan>{line}</cyan> - <level>{message}</level>'
            ...
            with custom_logger.catch(level="CRITICAL", message="exception test message"):
